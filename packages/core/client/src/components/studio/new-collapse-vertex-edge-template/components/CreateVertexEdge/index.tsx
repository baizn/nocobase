...
const TableEditForm = (props: Props) => {
  const { field: PField } = props
    ...
    return [
      {
        ...
        render(text: any, field: { name: string | number; fieldKey: React.Key; }) {
          return <Form.Item
            ...
            name={[PField.name, field.name, 'name']}
            fieldKey={[field.fieldKey, 'name']}
          >
            <Input placeholder='Please input Name!' allowClear style={{ width: '100%' }} disabled={disable} />
          </Form.Item>
        }
      },
      {
        ...
        render(text: any, field: { name: string | number; fieldKey: React.Key; }) {
          return <Form.Item
            name={[PField.name, field.name, 'type']}
      ...
      },
      {
        ...
        render(text: any, field: { name: string | number; fieldKey: React.Key; }) {
          return <Form.Item
            ...
            name={[PField.name, field.name, 'primaryKey']}
      ...
      },
      {
        ...
        render(text: any, field: { name: string | number; fieldKey: React.Key; }) {
          return <Form.Item
            name={[PField.name, field.name, 'comments']}
            fieldKey={[field.fieldKey, 'comments']}
          >
            <Input placeholder='请输入comments' allowClear style={{ width: '100%' }} disabled={disable} />
          </Form.Item>
        }
      },
      {
        ...
        render(text: any, field: { name: any; }) {
          return (
            <Button size='small' danger disabled={disable} style={{ position: 'absolute', top: '22px' }} onClick={() => remove(field.name)} >Delete</Button>
          )
        }
      }
    ]
  ...
  return (
    <div style={{ padding: '16px'}}>
      <Row>
        <Col span={12}>
          <Form.Item
            label='Label'
            {...PField}
            name={[PField.name, "label"]}
            rules={[{ required: true, message: 'Please input Label!' }]}
            tooltip={type == 'Vertex' ? 'Vertex Label' : 'Edge Label '}
            wrapperCol={{ span: 23 }}
          >
        ...
        {
          (isEdit == 'dataimport' || isEdit == '') ?
            <Col span={12}>
              <Form.Item
                label='Data Source'
                {...PField}
                name={[PField.name, "datasource"]}
                tooltip={'Data Source'}
              >
                <Input disabled />
              </Form.Item>
              ...
              <>
                <Col span={12}>
                  <Form.Item
                    ...
                    wrapperCol={{ span: isEdit !== 'dataimport' ? 0 : 23 }}
                  >
                    <Select
      ...
      <Form.List {...props.field} name={[PField.name, "table_list"]}>
        {(fields, { add, remove }) => {
          // 将Table视为 Form.List 中循环的 Form.Item
          return (
            <>
              <Form.Item
                tooltip={type == 'Vertex' ? 'Vertex Properties' : 'Edge Properties'}
              ...
              >
                ...
                />
                <div>
                  {isAddProperties ? null :
                    <Form.Item>
                      <Button disabled={isAddProperties || disable} style={{ width: '100%', color: '#6A6B71', marginTop: '8px' }} type="dashed" onClick={() => add()} block icon={<PlusOutlined />}>
                        Add Property
                      </Button>
                    </Form.Item>
                  }
